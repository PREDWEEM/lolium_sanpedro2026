...
if HAY_NUMBA:
    dtw_distance = njit(dtw_distance_python)
else:
    dtw_distance = dtw_distance_python

def calculate_tt_vectorized(t_array, t_base, t_opt, t_crit):
    tt = np.zeros_like(t_array)
    mask1 = (t_array > t_base) & (t_array <= t_opt)
    tt[mask1] = t_array[mask1] - t_base
    mask2 = (t_array > t_opt) & (t_array < t_crit)
    tt[mask2] = (t_array[mask2] - t_base) * ((t_crit - t_array[mask2]) / (t_crit - t_opt))
    return tt
...
def calcular_et0_hargreaves(jday, tmax, tmin, latitud=-38.45):
    ...
    ra_mm = ra / 2.45
    return np.maximum(0.0023 * ra_mm * (((tmax + tmin) / 2.0) + 17.8) * np.sqrt(np.maximum(tmax - tmin, 0)), 0)

def balance_hidrico_superficial(prec, et0, w_max=20.0, ke_suelo_max=0.4):
    n = len(prec)
    w = np.zeros(n)
    w[0] = w_max / 2.0  
    for i in range(1, n): w[i] = max(0.0, min(w_max, w[i-1] + prec[i] - (et0[i] * (ke_suelo_max * (w[i-1] / w_max)))))
    return w
...
with st.spinner("⏳ Procesando datos y modelos matemáticos..."):
    ...
    if df is not None and modelo_ann is not None:
        ...
        df["DG"] = calculate_tt_vectorized(df["Tmedia"].values, t_base_val, t_opt_max, t_critica)
        ...
        with tab4:
            ...
            fig_bio = go.Figure(go.Scatter(x=x_temps, y=calculate_tt_vectorized(x_temps, t_base_val, t_opt_max, t_critica), mode='lines', line=dict(color='#2563eb', width=4), fill='tozeroy'))
