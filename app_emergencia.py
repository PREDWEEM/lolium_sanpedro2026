...
# Actualización:
# - UI: Selector de rastrojo continuo con tarjeta HTML nativa (fondo blanco garantizado).
...
st.title("🌾 PREDWEEM LOLIUM - TRES ARROYOS (BA) lat=-38.378223")
...
with st.expander("📂 1. Datos del Lote", expanded=True):
    ...
    with col_rastrojo:
        ...
        # 3. Tarjeta HTML/CSS inyectada (A prueba de fallos visuales)
        html_card = f"""
        <div style="
            background-color: #ffffff;
            padding: 15px 20px;
...
# ---------------------------------------------------------
# 5. MOTOR DE CÁLCULO (LÓGICA MECANÍSTICA)
# ---------------------------------------------------------
if df is not None and modelo_ann is not None:
    ...
    # Balance Hídrico Superficial (BHS)
    ...
    with tab1:
        ...
        if fecha_inicio_ventana:
            dias_stress = len(df[(df["Fecha"] >= fecha_inicio_ventana) & (df["Tmedia"] > t_opt_max)])
    ...
    with tab3:
        ...
        if not df_obs.empty and df_obs["EMERREL"].sum() > 0:
            ...
            dists = [dtw_distance(obs_norm, m[JD_COM <= jd_corte] / (m[JD_COM <= jd_corte].max() or 1)) for m in cluster_model["curves_interp"]]
    ...
    with tab4:
        ...
        st.plotly_chart(go.Figure().add_trace(go.Scatter(x=x_temps, y=y_tt, mode='lines', line=dict(color='#2563eb', width=4), fill='tozeroy')), use_container_width=True)
